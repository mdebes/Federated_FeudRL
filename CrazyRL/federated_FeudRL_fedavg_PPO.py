...
def train_feudal_episode(manager, 
    ...
    # Buffers for Worker-PPO
    worker_states  = {agent: [] for agent in agent_keys}
    worker_actions = {agent: [] for agent in agent_keys}
    ...
    while not done:
        ...
        if (steps - 1) % manager_interval == 0:
            ...
            # PPO states list
            manager_states_list.append(global_state_tensor.detach())
            ...
            # PPO actions list
            manager_actions_list.append(m_sample.detach())
            ...
            manager_log_probs_list.append(m_log_prob.detach())
            manager_entropies_list.append(m_entropy.detach())
        ...
        for i, agent in enumerate(agent_keys):
            ...
            # Record state & action for Worker-PPO
            worker_states[agent].append(worker_input_tensor.detach())
            ...
            # Save sampled action
            worker_actions[agent].append(w_sample.detach())           

            log_prob = w_dist.log_prob(w_sample).sum(dim=1)
            entropy = w_dist.entropy().sum(dim=1)
            worker_log_probs[agent].append(log_prob.detach())
            worker_entropies[agent].append(entropy.detach())
    ...
    # === 1) Prepare returns & advantages ===
    # Manager:
    manager_values.append(torch.zeros(1,1,device=device))
    m_values = torch.cat(manager_values, dim=0).squeeze(-1)      # [T+1]
    m_returns  = torch.FloatTensor(compute_gae(manager_rewards, m_values.tolist(), gamma, lam)).to(device)  # [T]
    m_advantages = (m_returns - m_values[:-1].detach()).detach()    # [T]
    m_logp_old = torch.stack(manager_log_probs_list)            # [T, num_agents]
    
    manager_states = torch.cat(manager_states_list, dim=0)        # [T, state_size]
    manager_actions = torch.stack(manager_actions_list)          # [T, num_workers, goal_size]


    # === 2) Manager PPO updates ===
    for _ in range(manager_ppo_epochs):
        m_mean, m_std = manager.policy(manager_states)
        m_dist  = torch.distributions.Normal(m_mean, m_std)
        # Re‐compute log probs of the _same_ sampled actions
        m_logp = m_dist.log_prob(manager_actions).sum(dim=[1,2])    # [T]
        ratio  = torch.exp(m_logp - m_logp_old.sum(-1))
        clip_ratio = torch.clamp(ratio, 1-ppo_clip_eps, 1+ppo_clip_eps)
        actor_loss = -torch.min(ratio*m_advantages, clip_ratio*m_advantages).mean()
        value_pred = manager.value(manager_states).squeeze(-1)
        critic_loss = value_coef * F.mse_loss(value_pred, m_returns)
        entropy_loss = -entropy_coef * m_dist.entropy().sum(dim=-1).mean()
        loss = actor_loss + critic_loss + entropy_loss

        manager.policy_optimizer.zero_grad()
        manager.value_optimizer.zero_grad()
        loss.backward()
        torch.nn.utils.clip_grad_norm_(manager.policy_net.parameters(), 5.0)
        torch.nn.utils.clip_grad_norm_(manager.value_net.parameters(), 5.0)
        manager.policy_optimizer.step()
        manager.value_optimizer.step()

    # === 3) Worker PPO updates (shared network per client) ===
    # For each agent, flatten its trajectory and update its shared policy/value net
    for agent in agent_keys:
        # Build returns & advantages as before…
        worker_values[agent].append(torch.zeros(1,1,device=device))
        w_values = torch.cat(worker_values[agent], dim=0).squeeze(-1)   # [T+1]
        w_returns = torch.FloatTensor(compute_gae(worker_rewards[agent], w_values.tolist(), gamma, lam)).to(device)
        w_advantages = (w_returns - w_values[:-1].detach()).detach()    # [T]
        w_logp_old = torch.stack(worker_log_probs[agent]).squeeze(-1) # [T]
    
        # PPO Flatten the collected states & actions
        states  = torch.cat(worker_states[agent],  dim=0)  # [T, state_size]
        actions = torch.cat(worker_actions[agent], dim=0)  # [T, action_size]
        
        # PPO updates
        for _ in range(worker_ppo_epochs):
            w_mean, w_std = workers[agent].policy(states)
            w_dist = torch.distributions.Normal(w_mean, w_std)
            w_logp = w_dist.log_prob(actions).sum(dim=-1)
            ratio = torch.exp(w_logp - w_logp_old)
            clip_ratio = torch.clamp(ratio, 1-ppo_clip_eps, 1+ppo_clip_eps)
            actor_loss = -torch.min(ratio*w_advantages, clip_ratio*w_advantages).mean()

            value_pred = workers[agent].value(states).squeeze(-1)
            critic_loss = value_coef * F.mse_loss(value_pred, w_returns)

            entropy_loss = -entropy_coef * w_dist.entropy().sum(dim=-1).mean()

            # FedProx proximal term on this client’s shared worker network
            prox = compute_fedprox_term(workers[agent], global_worker_state, mu)

            loss = actor_loss + critic_loss + entropy_loss + prox

            workers[agent].policy_optimizer.zero_grad()
            workers[agent].value_optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(workers[agent].policy_net.parameters(), 5.0)
            torch.nn.utils.clip_grad_norm_(workers[agent].value_net.parameters(), 5.0)
            workers[agent].policy_optimizer.step()
            workers[agent].value_optimizer.step()
    
    # Return metrics.
    return total_episode_reward, caught_train, steps
...
def federated_feudal_training(num_clients=4,
    ...
    # The manager state: (3 numbers per worker + 3 for target + 3 velocity) 
    ...
    for comm_round in range(comm_rounds):
        ...
        for client_idx in range(num_clients):
            ...
            for ep in range(episodes_per_round):
                ...
                try:
                    ep_reward, caught, steps = train_feudal_episode(
                        global_manager,
                        workers,
                        env,
                        ...
                        lam=lam,
                        manager_interval=3,
                        manager_ppo_epochs=manager_ppo_epochs,
                        worker_ppo_epochs=worker_ppo_epochs,
    ...
    np.savez(
        f"training_metrics__fedavg_ppo_{timestamp}.npz",
    ...
    plt.title('Federated FeudRL FedAvg PPO Training Progress')
...
def visualize_episode(agent_trajectories, target_trajectory, total_reward, caught, axis_lim=5):
    ...
    # Colors per agent
    colors = {'agent_0': 'darkred', 'agent_1': 'darkorange'}
    ...
    ax.set_title('FedAvg Episode Trajectories')
    # Legend with handles
...
def objective(trial):
    ...
    manager_ppo_epochs = trial.suggest_int("manager_ppo_epochs", 2, 10)
    worker_ppo_epochs  = trial.suggest_int("worker_ppo_epochs", 2, 10)
    ppo_clip_eps        = trial.suggest_float("ppo_clip_eps", 0.1, 0.3)
...
worker_lr = 1.16e-05
entropy_coef = 0.018
gamma = 0.94 # 1 to remove lr decay
comm_rounds = 1000
episodes_per_round = 50
mu= 9.37e-05# 0 to disable FedProx
gae_lam = 0.92 # 1 to disable GAE
manager_ppo_epochs = 6        # how many PPO epochs per manager update
worker_ppo_epochs  = 8        # how many PPO epochs per worker update
ppo_clip_eps       = 0.13      # PPO clipping ε
value_coef         = 0.15      # Critic loss weight

if carbontracker:
    tracker = EmissionsTracker(output_file="fedavg_ppo_emissions.csv")
    tracker.start()
...
if training:
    ...
    if __name__ == '__main__':       
        ...
        torch.save(global_manager.policy_net.state_dict(), 'global_manager_policy_fedavg_ppo.pth')
        torch.save(global_manager.value_net.state_dict(), 'global_manager_value_fedavg_ppo.pth')
        
        # Save the global worker model from one client (all clients are averaged).
        global_worker = list(client_worker_agents[0].values())[0]
        torch.save(global_worker.policy_net.state_dict(), 'global_worker_policy_fedavg_ppo.pth')
        torch.save(global_worker.value_net.state_dict(), 'global_worker_value_fedavg_ppo.pth')
...
if testing:
    ...
    global_manager.policy_net.load_state_dict(torch.load('global_manager_policy_fedavg_ppo.pth', map_location=device))
    global_manager.value_net.load_state_dict(torch.load('global_manager_value_fedavg_ppo.pth', map_location=device))
    ...
    global_worker.policy_net.load_state_dict(torch.load('global_worker_policy_fedavg_ppo.pth', map_location=device))
    global_worker.value_net.load_state_dict(torch.load('global_worker_value_fedavg_ppo.pth', map_location=device))
