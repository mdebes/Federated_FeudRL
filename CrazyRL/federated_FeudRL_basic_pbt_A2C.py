...
def federated_feudal_training(num_clients=4,
    ...
    for comm_round in range(comm_rounds):
        ...
        for client_dict in client_worker_agents:
            ...
            copy_model_params(w.policy_net, best_worker.policy_net)
            copy_model_params(w.value_net,  best_worker.value_net)
    ...
    plt.title('Federated FeudRL PBT A2C Light Training Progress')
...
worker_lr = 1.92e-05
entropy_coef = 0.02
gamma = 0.915 # 1 to remove lr decay
comm_rounds = 1000
episodes_per_round = 50

if carbontracker:
    tracker = EmissionsTracker(output_file="pbt_lite_emissions.csv")
    tracker.start()
...
if training:
    ...
    if __name__ == '__main__':       
        ...
        torch.save(global_manager.policy_net.state_dict(), 'global_manager_policy_pbt_light.pth')
        torch.save(global_manager.value_net.state_dict(), 'global_manager_value_pbt_light.pth')
        
        # Save the global worker model from one client (all clients are averaged).
        global_worker = list(client_worker_agents[0].values())[0]
        torch.save(global_worker.policy_net.state_dict(), 'global_worker_policy_pbt_light.pth')
        torch.save(global_worker.value_net.state_dict(), 'global_worker_value_pbt_light.pth')
...
if testing:
    ...
    global_manager.policy_net.load_state_dict(torch.load('global_manager_policy_pbt_light.pth', map_location=device))
    global_manager.value_net.load_state_dict(torch.load('global_manager_value_pbt_light.pth', map_location=device))
    ...
    global_worker.policy_net.load_state_dict(torch.load('global_worker_policy_pbt_light.pth', map_location=device))
    global_worker.value_net.load_state_dict(torch.load('global_worker_value_pbt_light.pth', map_location=device))
