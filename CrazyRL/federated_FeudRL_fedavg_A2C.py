...
def federated_feudal_training(num_clients=4,
    ...
    np.savez(
        f"training_metrics__fedavg_{timestamp}.npz",
    ...
    plt.title('Federated FeudRL FedAvg A2C Training Progress')
...
mu= 0.0003 # 0 to disable FedProx
gae_lam = 0.92 # 1 to disable GAE

if carbontracker:            
    tracker = EmissionsTracker(output_file="fedavg_a2c_emissions_{device}.csv")
    tracker.start()
...
if training:
    ...
    if __name__ == '__main__':       
        ...
        torch.save(global_manager.policy_net.state_dict(), 'global_manager_policy_fedprox.pth')
        torch.save(global_manager.value_net.state_dict(), 'global_manager_value_fedprox.pth')
        
        # Save the global worker model from one client (all clients are averaged).
        global_worker = list(client_worker_agents[0].values())[0]
        torch.save(global_worker.policy_net.state_dict(), 'global_worker_policy_fedprox.pth')
        torch.save(global_worker.value_net.state_dict(), 'global_worker_value_fedprox.pth')
...
if testing:
    ...
    global_manager.policy_net.load_state_dict(torch.load('global_manager_policy_fedprox.pth', map_location=device))
    global_manager.value_net.load_state_dict(torch.load('global_manager_value_fedprox.pth', map_location=device))
    ...
    global_worker.policy_net.load_state_dict(torch.load('global_worker_policy_fedprox.pth', map_location=device))
    global_worker.value_net.load_state_dict(torch.load('global_worker_value_fedprox.pth', map_location=device))
