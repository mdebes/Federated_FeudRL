...
class ManagerNetwork(nn.Module):
    def __init__(self, state_size, goal_size, num_agents, scale, hidden_size=128):
        ...
        self.scale = scale  # To match the environment’s coordinate range

    def forward(self, x):
        x = torch.relu(self.fc1(x))
        x = torch.relu(self.fc2(x))
        raw_mean = self.fc_mean(x)
        mean = torch.tanh(raw_mean) * self.scale  # scale output to match env range
...
class ManagerAgent:
    def __init__(self, state_size, goal_size, num_agents, scale, lr=5e-4):
        ...
        self.value_net = ManagerValueNetwork(state_size)
...
class WorkerAgent:
    def __init__(self, state_size, action_size, lr=5e-4):
        ...
        self.value_net = WorkerValueNetwork(state_size)
        self.policy_optimizer = optim.Adam(self.policy_net.parameters(), lr=lr)
        self.value_optimizer = optim.Adam(self.value_net.parameters(), lr=lr)
...
def train_feudal_episode(manager, workers, env, episode,
    ...
    manager_values.append(torch.zeros(1, 1))
    ...
    if episode >= curriculum_threshold:
        ...
        for t in range(len(manager_goals_list)):
            ...
            for idx, agent in enumerate(agent_keys):
                ...
                consistency_loss += F.mse_loss(predicted_goal, desired_goal)
                count += 1
        if count > 0:
