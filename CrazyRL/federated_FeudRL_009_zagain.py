...
class ManagerNetwork(nn.Module):
    ...
    def forward(self, x):
        ...
        # Modified code: allows z_mean to be in [-z_scale, z_scale] from 0.2 to 3
        z_mean = 0.2 + (torch.tanh(raw_mean[:, :, 2]) + 1) * self.z_scale
...
def federated_feudal_training(num_clients=4,
    ...
    global_manager = ManagerAgent(manager_state_size, goal_size, num_agents=agents_per_client,
                                  xy_scale=params["env_size"], z_scale=1.4, lr=lr, hidden_size=m_hidden_size)
    ...
    for _ in range(num_clients):
        ...
        worker_agent = WorkerAgent(worker_state_size, action_size, lr=lr, hidden_size=w_hidden_size)
...
def objective(trial):
    ...
    mu = trial.suggest_uniform("mu", 1e-5, 5e-4)
    lam = trial.suggest_uniform("lam", 0.90, 0.99)  # GAE lambda
    intrinsic_coef = trial.suggest_uniform("intrinsic_coef", 0.05, 0.2)
    consistency_lambda = trial.suggest_uniform("consistency_lambda", 0.01, 1.0)
    curriculum_rate = trial.suggest_uniform("curriculum_rate", 0.1, 0.5)
    fedprox_rate = trial.suggest_uniform("fedprox_rate", 0.5, 0.75)

    # Use a lower number of rounds to keep computation tractable.
    comm_rounds = 50         # Total communication rounds (max 1000 in full training)
    episodes_per_round = 5   # Episodes per communication round
    total_eps = comm_rounds * episodes_per_round

    # Call your federated_feudal_training function with the suggested hyperparameters.
    round_rewards, global_manager, client_worker_agents = federated_feudal_training(
        num_clients=4,
...
if training:
    ...
    if __name__ == '__main__':
        m_hidden_size = 256
        w_hidden_size = 256
        lr = 0.00024 # 5e-4 1e-5
        entropy_coef = 0.027
        gamma = 0.985 # 1 to remove lr decay
        comm_rounds = 2000
        episodes_per_round = 10
        total_eps = comm_rounds*episodes_per_round
        curriculum_rate = 0.15 # Next try higher cw. 1 to disable
        mu=0.0001 #  0 to disable FedProx. 
        fedprox_rate = 0.5
        gae_lam = 0.9
        intrinsic_coef = 0 #0.15. 0 to test environment direct rewards.
        consistency_lambda = 0.065 #0.01. next try: 0
        ...
        with open(file_name, "a", newline="") as f:
            ...
            values = ",".join([
                ...
                str(gae_lam),
                str(intrinsic_coef),
                str(consistency_lambda),
                f"{avg_last_100:.2f}"
            ])
            f.write(values + "\n")
        
        print("Hyperparameters and results appended to", file_name)
...
if finetunnning:
    ...
    study = optuna.create_study(direction="minimize")  # Minimizing the negative reward is equivalent to maximizing reward.
