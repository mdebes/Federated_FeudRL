...
def federated_feudal_training(num_clients=4,
    ...
    np.savez(
        f"training_metrics__pbt_ppo_{timestamp}.npz",
    ...
    plt.title('Federated FeudRL PBT PPO Training Progress')
...
worker_lr = 2.45e-05
entropy_coef = 0.014
gamma = 0.95 # 1 to remove lr decay
comm_rounds = 1000
episodes_per_round = 50
mu= 1.7e-05 # 0 to disable FedProx
gae_lam = 0.94 # 1 to disable GAE
manager_ppo_epochs = 2        # how many PPO epochs per manager update
worker_ppo_epochs  = 9        # how many PPO epochs per worker update
ppo_clip_eps       = 0.15      # PPO clipping ε
value_coef         = 0.25      # Critic loss weight
...
if training:
    ...
    if __name__ == '__main__':       
        ...
        torch.save(global_manager.policy_net.state_dict(), 'global_manager_policy_pbt_ppo.pth')
        torch.save(global_manager.value_net.state_dict(), 'global_manager_value_pbt_ppo.pth')
        
        # Save the global worker model from one client (all clients are averaged).
        global_worker = list(client_worker_agents[0].values())[0]
        torch.save(global_worker.policy_net.state_dict(), 'global_worker_policy_pbt_ppo.pth')
        torch.save(global_worker.value_net.state_dict(), 'global_worker_value_pbt_ppo.pth')
...
if testing:
    ...
    global_manager.policy_net.load_state_dict(torch.load('global_manager_policy_pbt_ppo.pth', map_location=device))
    global_manager.value_net.load_state_dict(torch.load('global_manager_value_pbt_ppo.pth', map_location=device))
    ...
    global_worker.policy_net.load_state_dict(torch.load('global_worker_policy_pbt_ppo.pth', map_location=device))
    global_worker.value_net.load_state_dict(torch.load('global_worker_value_pbt_ppo.pth', map_location=device))
